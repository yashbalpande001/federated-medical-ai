...
class RSNAFedProxClient(NumPyClient):
    """
    Flower Client for local client training on RSNA dataset partitions with FedProx proximal term:
    Loss = BinaryFocalLoss + (mu / 2) * ||w_local - w_global||^2
    """
    ...
    def fit(self, parameters: List[np.ndarray], config: Dict[str, str]) -> Tuple[List[np.ndarray], int, Dict]:
        """
        Trains local model on client's partition for epochs_per_round, adding FedProx proximal penalty:
        (mu / 2) * sum(||w_local - w_global||^2)
        """
        self.set_parameters(parameters)

        # Freeze copy of global parameters (w^t) for proximal loss calculation
        ...
        for _ in range(self.epochs_per_round):
            for images, labels in self.train_loader:
                ...
                # Calculate FedProx proximal term penalty: (mu / 2) * ||w_local - w_global||^2
                prox_term = 0.0
                if self.mu > 0.0:
                    for w_local, w_global in zip(self.model.parameters(), global_params):
                        prox_term += (w_local - w_global).pow(2).sum()
                ...
                total_prox_loss += float(prox_term.item()) * len(labels) if isinstance(prox_term, torch.Tensor) else 0.0
                total_samples += len(labels)

        avg_loss = total_loss / max(1, total_samples)
        avg_prox_loss = total_prox_loss / max(1, total_samples)
        updated_params = self.get_parameters(config={})

        return updated_params, len(self.dataset), {
            "train_loss": float(avg_loss),
            "prox_loss": float(avg_prox_loss),
            "mu": float(self.mu),
        }
